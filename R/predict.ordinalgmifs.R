...
function(object, neww=NULL, newx=NULL, model.select="AIC", ...) {
	...
	k<-length(unique(y))
	if (!is.null(neww)) 	neww<-as.matrix(neww)
	...
	if (object$probability.model=="Cumulative") {
		...
		# pi.z is the matrix of estimated probabilities from the
		#   objectted model
	    ...
	    if (link=="logit") {
	        for (i in 1:k) {
				if (i==1) {
			    	...
			    	pi[,i] <- exp(z[,i]) / (1+exp(z[,i])) - exp(z[,i-1]) / (1+exp(z[,i-1]))
				} else if (i==k) {
				pi[,i] <- 1 - exp(z[,i-1]) / (1 + exp(z[, i-1]))
				}
    		}
    	} else if (link=="probit") {
    	    for (i in 1:k) {
			...
			for (i in 1:k) {
				if (i==1) {
					...
					pi[,i]<-exp(-exp(z[,i-1])) - exp(-exp(z[,i]))
				} else if (i==k) {
					pi[,i]<-exp(-exp(z[,i-1]))
				}
			}
		...
		if (link=="logit" | link=="cloglog") {
			...
			if (k > 3) {
				for (i in 3:(k-1)) {
					pi[, i] <- G(alpha[i] + Xb, link) *(1 - matrix(apply(pi[,1:(i-1)], 1, sum), nrow=nrow(pi), byrow=T))
				}
			}		
		} else if (link=="probit") {	
    		...
    		if (k>3) {
    			for (i in 3:(k-1)) {
    				pi[,i]<-pnorm(alpha[i]+Xb)*(1-matrix(apply(pi[,1:(i-1)],1,sum),nrow=nrow(pi),byrow=T))
				}
			}
		}
		pi[, k] <- 1 - matrix(apply(pi[, 1:(k-1)], 1, sum),nrow=nrow(pi),byrow=T)
	} else if (object$probability.model=="BackwardCR") {
		pi <- matrix(0, nrow=n, ncol=k)  
		if (link=="logit" | link=="cloglog") {
			pi[,k] <- G(alpha[k-1] + Xb, link)
			pi[,k-1] <- G(alpha[k-2] + Xb, link) * (1 - pi[,k])
			if (k > 3) {
				for (i in (k-2):2) {
					pi[,i] <- G(alpha[i-1] + Xb, link) *(1 - matrix(apply(pi[,k:(i+1)], 1, sum), nrow=nrow(pi), byrow=T))
				}
			}
		} else if (link=="probit") {
    		...
    		if (k>3) {
    			for (i in (k-2):2) {
    				pi[,i]<-pnorm(alpha[i-1]+Xb)*(1-matrix(apply(pi[,k:(i+1)],1,sum),nrow=nrow(pi),byrow=T))
				}
			}
		}
		pi[,1] <- 1 - matrix(apply(pi[,k:2],1,sum),nrow=nrow(pi),byrow=T)
	} else if (object$probability.model=="Stereotype") {
		eta<-matrix(0,ncol=k-1,nrow=n)
		for (i in 1:(k-1)) {
			eta[,i]<-exp(alpha[i] + phi[i]*Xb)
		}
      	pik<- 1- apply(eta,1,sum)/(1+apply(eta,1,sum))
		pi<-matrix(0,ncol=k,nrow=n)
		pi[,k]<- pik
		pi[,1:(k-1)]<-eta*pik
	}
